...
Snažíme se nadšení o těchto změnách obnovení balíčků, jako by to zavedlo nového pracovního postupu. Pokud chcete vynechat vlastních balíčků od správy zdrojového kódu jednoduše nemáte potvrzení `packages` složky. Visual Studio uživatele, kteří otevřete a sestavte řešení zobrazí balíčky automaticky obnoví. Pro sestavení příkazového řádku, jednoduše vyvolání `nuget.exe restore` před vyvoláním `msbuild`. Už nemusíte nezapomeňte použít gesto "Povolit obnovení balíčků NuGet" na řešení a už budeme potřebovat změnit projekty pro úpravu sestavení. A také dostaneme mnohem lepší prostředí pro balíčky, které zahrnují MSBuild importy, zejména pro importy přidány prostřednictvím NuGet nejnovější funkce pro [automaticky import props/cíle soubory](../release-notes/nuget-2.5.md#automatic-import-of-msbuild-targets-and-props-files) ze složky \build.
