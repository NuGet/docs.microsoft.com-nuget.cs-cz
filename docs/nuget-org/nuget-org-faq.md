...
- Stáhnout WinMTR z[http://winmtr.net/download/](http://winmtr.net/)
- Jako `api.nuget.org` název hostitele zadejte a klikněte na **Spustit**.
...
Poznámka: může být potřeba, abyste proměnnou `HTTP_PROXY` prostředí nastavili na `http://127.0.0.1:8888` směrování provozu NuGet přes Fiddler.
...
Pokud chcete změnit účet Microsoft pro uživatele NuGet.org, postupujte podle následujících kroků. Znamená, že vaše účet Microsoft s `account1@outlook.com` e-mailem jsou spojené s vaším `MyNuGetAccount`účtem NuGet.org s uživatelským jménem. Chcete změnit přihlašovací údaje k jinému účet Microsoft e-mailem`account2@outlook.com`
...
1. Pokud se zobrazí chyba níže, přečtěte si téma [účet Microsoft je propojeno s jiným účtem NuGet.org](#microsoft-account-is-linked-with-another-nugetorg-account) , kde najdete další podrobnosti.
    >_Nepovedlo se aktualizovat účet Microsoft pomocí ' <account2@outlook.com>Account2 '. K tomu může dojít, pokud je už propojený s jiným účtem NuGet. Pokud chcete získat další informace, obraťte se na podporu._
...
Řekněme, že jste se pokoušeli změnit účet Microsoft přihlašovací `account1@outlook.com` jméno pro uživatele NuGet.org s `MyNuGetAccount1` uživatelským jménem na jiný `account2@outlook.com`účet Microsoft s e-mailem. A zobrazí se chyba výše.

**Co výše chyba znamená?**

Znamená to, že existuje jiný účet NuGet.org, který je spojený s účet Microsoft, na kterou se pokoušíte změnit. v předchozím příkladu je účet Microsoft s e- `<account2@outlook.com>` mailem přidružen k jinému účtu NuGet.org s, řekněme, username. `MyNuGetAccount2`.
...
Přihlaste se k druhé účet Microsoft na přihlašovací(https://www.nuget.org/users/account/LogOn?returnUrl=%2F# "stránce")přihlašovací [stránky]. Tím se přihlásíte k účtu NuGet.org, který je aktuálně přidružený k druhému účet Microsoft. Pak můžete zobrazit nahrané balíčky a provádět správu účtů na tomto účtu.
...
Pokud si chcete být jisti, že se druhý účet NuGet.org ještě netýká, a přesto chcete znovu použít přidružený účet Microsoft k `account2@outlook.com`e-mailu. 

Přidružení mezi účtem účet Microsoft a NuGet.org můžete uvolnit tak, že odstraníte účet NuGet.org.
1. Použijte postup [odstranění uživatele](#how-to-delete-my-nugetorg-account) pro druhý účet `MyNuGetAccount2`NuGet.org. 
...
V e-mailu `account3@outlook.com`budete muset vytvořit nebo použít třetí účet Microsoft, řekněme. 
1. Nejdřív byste se měli přihlásit s druhým účet Microsoft `account2@outlook.com` na NuGet.org. Podle výše uvedených kroků změňte přidružená přihlášení a přidružte třetí účet Microsoft k tomuto účtu NuGet.org.
1. Až to bude hotové, vaše druhá `account2@outlook.com` účet Microsoft s e-mailem bude přidružit k vašemu `MyNuGetAccount1`prvnímu účtu NuGet.org. Pomocí výše uvedených kroků změňte přihlašovací údaje Microsoftu na druhý účet Microsoft.
...
Pokud jste se pokusili přihlásit pomocí účet Microsoft, řekněme pomocí e-mailu `account1@outlook.com` a zobrazí se chyba podobná této:
> _Účet s e-mailemaccount1@outlook.com' ' je propojen s jiným účtem Microsoft._
>
> _Pokud chcete aktualizovat propojený účet Microsoft můžete to provést ze stránky nastavení účtu._

**Co výše chyba znamená?**

Při vytvoření účtu v NuGet.org je k tomuto účtu přidružena e-mailová adresa pro komunikaci. To je obvykle stejné jako e-mailová adresa, která se používá pro přidruženou účet Microsoft. Můžete ale zvolit, že chcete zadat jinou e-mailovou adresu pro komunikaci. To znamená, `account2@outlook.com` že můžete mít jinou účet Microsoft, řekněme, že se jedná o propojený s účtem NuGet.org s e-mailovou `account1@outlook.com`adresou pro komunikaci.

Takže výše uvedená chyba znamená, že už existuje účet NuGet.org s e-mailovou adresou `account1@outlook.com` pro komunikaci `account1@outlook.com`, ale je přidružený k jinému účet Microsoft s e-mailem **, který** není.

**Návody zjistit, které účet Microsoft je propojeno s tímto účtem NuGet.org?**

K zjištění, které účet Microsoft jsou propojeny s účtem NuGet.org s e-mailovou adresou `account1@outlook.com`, byste měli použít postup [pomoc při přihlašování](#which-microsoft-account-is-linked-to-my-nugetorg-account) .
...
Pokud jste se pokusili použít [pomoc](#which-microsoft-account-is-linked-to-my-nugetorg-account) při přihlašování a nemáte přístup k účet Microsoft, která je přidružená k vašemu účtu NuGet.org, postupujte podle následujících kroků, abyste provedli propojení nového účet Microsoft s účtem NuGet.org.
...
2. Pokud jste zapomněli své uživatelské jméno a heslo pro svůj účet NuGet.org, postupujte podle [pokynů pro obnovení přihlašovacích](#how-to-recover-nugetorg-password-login)údajů k heslu.
...
5. **POZNÁMKA**: Pro přihlášení se zadaným účet Microsoft prosím ignorujte instrukci. Svůj účet NuGet.org teď můžete propojit s ostatními přihlašovacími údaji Microsoftu.
...
1. **Požadavek**: Abyste se mohli použít jako správce účtu org, musíte nejdřív vytvořit jednotlivý účet na NuGet.org. Pokud ho ještě nemáte, [vytvořte prosím nový účet NuGet.org](individual-accounts.md) .
...
5. Přejděte do [ (Nastavení)https://www.nuget.org/account/transform](https://www.nuget.org/account/transform) (Integrace a služby). To vám umožní převést účet NuGet.org na org bez propojení účet Microsoft.
...
Pokud se v průběhu procesu přihlášení k doméně vašeho e-mailového účtu (@yourdomain.com) zobrazuje chyba, přečtěte si následující postup obnovení účtu NuGet.org.
...
K tomu může dojít, pokud jste vy nebo někdo z vaší organizace @yourdomain.com (s e-mailovou adresou) zaregistrovali jednu z integrovaných služeb AAD nebo [pro Azure Active Directory samoobslužnou registraci](https://docs.microsoft.com/en-us/azure/active-directory/users-groups-roles/directory-self-service-signup), která pro službu vytvořila tohoto nespravovaného tenanta. použitá doména účet Microsoft (@yourdomain.com v případě). 
...
Pokud se chcete přihlásit k NuGet.org pomocí účet Microsoft (@yourdomain.com), budete vy (nebo správce vaší společnosti) muset převzít vlastnictví AAD tím, že provedete ověření DNS pro ověřování uživatelů pomocí e-mailové@yourdomain.comadresy. Postupujte prosím podle kroků pro [převzetí správce domén](https://docs.microsoft.com/en-us/azure/active-directory/users-groups-roles/domains-admin-takeover) zdokumentované službou Azure Active Directory. Až to uděláte, vaše normální přihlášení by mělo začít pracovat.

**Nechci všechno udělat, jaký je druhý způsob, jak obnovit svůj účet?**

Můžete [vytvořit](https://www.microsoft.com/en-us/account) nový účet Microsoft (s e-mailem , který není @yourdomain.comspojený s). Postupujte podle kroků uvedených v části [obnovení účtu NuGet.org](#unable-to-use-microsoft-login-how-do-i-recover-my-nugetorg-account) .
