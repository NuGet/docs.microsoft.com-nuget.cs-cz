...
Chování NuGet doprovází Akumulovaná nastavení v jedné nebo více `NuGet.Config` soubory (XML), které může existovat projektu - uživatel- a úrovně platná pro celý počítač. Globální konfiguraci `NuGetDefaults.Config` souboru (2.7 +) také konkrétně nakonfiguruje zdroje balíčků. Nastavení se vztahují na všechny příkazy vydané v rozhraní příkazového řádku, konzoly Správce balíčků a uživatelského rozhraní Správce balíčků.

V tomto tématu:

- [Umístění souboru NuGet.Config a používá](#config-file-locations-and-uses)
- [Změna nastavení](#changing-config-settings)
...
A `NuGet.Config` soubor je jednoduchý textový soubor XML obsahující dvojice klíč/hodnota, jak je popsáno v [nastavení konfigurace NuGet](../Schema/nuget-config-file.md) tématu.
...
Windows: % ProgramFiles (x86) %\NuGet\Config (2.7 NuGet do NuGet 3.x: % PROGRAMDATA%\NuGet\NuGetDefaults.Config) Mac/Linux: $XDG_DATA_HOME (obvykle ~/.local/share) 

### <a name="nugetdefaultsconfig-settings"></a>Nastavení NuGetDefaults.Config

- `packageSources`: Tato kolekce nemá stejný význam jako `packageSources` v regulárním konfigurační soubory a určuje výchozí zdroje v jejich upřednostňované pořadí. Pokud toto nastavení existuje v `NuGetDefaults.Config`, pak NuGet nepoužívá nuget.org jako výchozí zdroj balíčku. Správce proto měli jistotu, že všechny, kteří používají tento soubor ve spolupráci s stejného zdroje a nevyužívá nuget.org v případě potřeby.
