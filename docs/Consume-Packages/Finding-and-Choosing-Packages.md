...
- [Hledání balíčků](#finding-packages)
- [Vyhodnocení balíčky](#evaluating-packages)
- [Syntaxe vyhledávání](#search-syntax)
...
1. Stáhněte si balíček z jeho stránky na nuget.org pomocí **ruční stažení** v části **informace**. Změňte příponu z `.nupkg` k `.zip`a otevřete soubor a zkontrolujte obsah jeho `lib` složky. Existuje uvidíte podsložky pro každou z podporovaných rozhraní, kde je každý podsložku s názvem se Přezdívka cílový framework (TFM; najdete v části [cílové rozhraní](../schema/Target-Frameworks.md)). Pokud se zobrazí žádné podsložky `lib` a pouze jeden soubor DLL a potom jste musí pokusíte nainstalovat balíček ve vašem projektu a zjistit jeho kompatibility.
