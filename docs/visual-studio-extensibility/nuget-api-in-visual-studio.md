...
---
# <a name="nuget-api-in-visual-studio"></a>Rozhraní API NuGetu v sadě Visual Studio

Kromě uživatelského rozhraní a konzoly Správce balíčků v aplikaci Visual Studio nástroj NuGet také exportuje některé užitečné služby prostřednictvím [Managed Extensibility Framework (MEF)](/dotnet/framework/mef/index). Toto rozhraní umožňuje ostatním komponentám v aplikaci Visual Studio pracovat s NuGet, které lze použít k instalaci a odinstalaci balíčků a k získání informací o nainstalovaných balíčcích.

V průběhu let NuGet přidalo mnoho služeb, které jsou umístěny v `NuGet.VisualStudio` oboru názvů v `NuGet.VisualStudio.dll` sestavení:

Od NuGet 3.3 + vyexportuje NuGet následující

- [`IRegistryKey`](#iregistrykey-interface): Metoda, která načte hodnotu z podklíče registru. (3.3 +)
- [`IVsCredentialProvider`](#ivscredentialprovider-interface) Obsahuje metody pro získání přihlašovacích údajů pro operace NuGet. (4.0 +)
- [`IVsFrameworkCompatibility`](#ivsframeworkcompatibility-interface) Obsahuje metody pro zjišťování platforem a kompatibilitu mezi platformami. (4.0 +)
- [`IVsFrameworkCompatibility2`](#ivsframeworkcompatibility2-interface) Obsahuje metody pro zjišťování platforem a kompatibilitu mezi platformami. (4.0 +)
- [`IVsFrameworkCompatibility3`](#ivsframeworkcompatibility3-interface) Obsahuje metody pro zjišťování platforem a kompatibilitu mezi platformami. (5,8 +)
- [`IVsFrameworkParser`](#ivsframeworkparser-interface) Rozhraní pro práci s převodem mezi řetězci a [architekturou](/dotnet/api/system.runtime.versioning.frameworkname) (4.0 +)
- [`IVsFrameworkParser2`](#ivsframeworkparser2-interface) Rozhraní pro analýzu .NET Framework řetězců. Viz [NuGet – IVsFrameworkParser](https://aka.ms/NuGet-IVsFrameworkParser). (5,8 +)
- [`IVsGlobalPackagesInitScriptExecutor`](#ivsglobalpackagesinitscriptexecutor-interface) Spouštění skriptů PowerShellu z balíčků v řešení (4.0 +)
- [`IVsNuGetFramework`](#ivsnugetframework-interface) Typ, který představuje komponenty pro moniker rozhraní .NET Target Framework. (5,8 +)
- [`IVsPackageInstaller`](#ivspackageinstaller-interface): Metody instalace balíčků NuGet do projektů. (3.3 +)
- [IVsPackageInstaller2](#ivspackageinstaller2-interface) Obsahuje metodu pro instalaci nejnovější verze jednoho balíčku do projektu v rámci aktuálního řešení.
- [`IVsPackageInstallerEvents`](#ivspackageinstallerevents-interface): Události pro instalaci nebo odinstalaci balíčku. (3.3 +)
- [`IVsPackageInstallerProjectEvents`](#ivspackageinstallerprojectevents-interface): Dávkové události pro instalaci nebo odinstalaci balíčku. (3.3 +)
- [`IVsPackageInstallerServices`](#ivspackageinstallerservices-interface): Metody načítající nainstalované balíčky v aktuálním řešení a pro kontrolu, zda je daný balíček nainstalován v projektu. (3.3 +)
- [`IVsPackageManagerProvider`](#ivspackagemanagerprovider-interface): Metody poskytnutí alternativních návrhů správce balíčků pro balíček NuGet. (3.3 +)
- [`IVsPackageMetadata`](#ivspackagemetadata-interface): Metody načtení informací o nainstalovaném balíčku. (3.3 +)
- [`IVsPackageProjectMetadata`](#ivspackageprojectmetadata-interface): Metody pro načtení informací o projektu, kde jsou spouštěny akce NuGet. (3.3 +)
- [`IVsPackageRestorer`](#ivspackagerestorer-interface): Metody obnovení balíčků nainstalovaných v projektu. (3.3 +)
- [`IVsPackageSourceProvider`](#ivspackagesourceprovider-interface): Metody načítající seznam zdrojů balíčků NuGet. (3.3 +)
- [`IVsPackageUninstaller`](#ivspackageuninstaller-interface): Metody odinstalace balíčků NuGet z projektů. (3.3 +)
- [`IVsPathContext`](#ivspathcontext-interface) Informace o cestě NuGet specifické pro aktuální kontext (např. kontext projektu) (4.0 +)
- [`IVsPathContext2`](#ivspathcontext2-interface) Informace o cestě NuGet specifické pro aktuální kontext (např. kontext projektu) (5.0 +)
- [`IVsPathContextProvider`](#ivspathcontextprovider-interface) Továrna pro inicializaci instancí [IVsPathContext](#ivspathcontext-interface) . (4.0 +)
- [`IVsPathContextProvider2`](#ivspathcontextprovider2-interface) Továrna pro inicializaci instancí [IVsPathContext2](#ivspathcontext2-interface) . (5.0 +)
- [`IVsProjectJsonToPackageReferenceMigrateResult`](#ivsprojectjsontopackagereferencemigrateresult-interface)  Obsahuje výsledek operace migrace na starší verzi project.jsprojektu (4.3 +).
- [`IVsProjectJsonToPackageReferenceMigrator`](#ivsprojectjsontopackagereferencemigrator-interface) Obsahuje metody pro migraci project.jsna základě starší verze projektu do projektu založeného na PackageReference. (4.3 +)
- [`IVsSemanticVersionComparer`](#ivssemanticversioncomparer-interface) Rozhraní pro porovnání dvou neprůhledných řetězců verzí tím, že je považuje za sémantiku NuGet (4.0 +)
- [`IVsTemplateWizard`](#ivstemplatewizard-interface): Určeno pro šablony projektů a položek pro zahrnutí předem nainstalovaných balíčků; Toto rozhraní není *určeno k* vyvolání z kódu a nemá žádné veřejné metody. (3.3 +)

## <a name="using-nuget-services"></a>Používání služby NuGet

1. Nainstalujte [`NuGet.VisualStudio`](https://www.nuget.org/packages/NuGet.VisualStudio) balíček do projektu, který obsahuje `NuGet.VisualStudio.dll` sestavení.

    Po instalaci balíček automaticky nastaví vlastnost **Embed Interop Types** odkazu na sestavení na **hodnotu true**. Díky tomu je váš kód odolný proti změnám verze, když se uživatelé aktualizují na novější verze NuGetu.

> [!Warning]
> Nepoužívejte žádné jiné typy kromě veřejných rozhraní v kódu a neodkazujte na jiná sestavení NuGet, včetně `NuGet.Core.dll` .

1. Chcete-li použít službu, importujte ji pomocí [importovaného atributu MEF](/dotnet/framework/mef/index#imports-and-exports-with-attributes)nebo prostřednictvím [služby IComponentModel](/dotnet/api/microsoft.visualstudio.componentmodelhost.icomponentmodel).
...
Pro referenci je zdrojový kód pro NuGet. VisualStudio obsažený v [úložišti NuGet. clients](https://github.com/NuGet/NuGet.Client/tree/dev/src/NuGet.Clients/NuGet.VisualStudio).

## <a name="iregistrykey-interface"></a>Rozhraní IRegistryKey
...
## <a name="ivscredentialprovider-interface"></a>Rozhraní IVsCredentialProvider
...
## <a name="ivsframeworkcompatibility-interface"></a>Rozhraní IVsFrameworkCompatibility
...
## <a name="ivsframeworkcompatibility2-interface"></a>Rozhraní IVsFrameworkCompatibility2
...
## <a name="ivsframeworkcompatibility3-interface"></a>Rozhraní IVsFrameworkCompatibility3
...
## <a name="ivsframeworkparser-interface"></a>Rozhraní IVsFrameworkParser
...
## <a name="ivsframeworkparser2-interface"></a>Rozhraní IVsFrameworkParser2
...
## <a name="ivsglobalpackagesinitscriptexecutor-interface"></a>Rozhraní IVsGlobalPackagesInitScriptExecutor
...
## <a name="ivsnugetframework-interface"></a>Rozhraní IVsNuGetFramework
...
## <a name="ivspackageinstaller-interface"></a>Rozhraní IVsPackageInstaller
...
## <a name="ivspackageinstaller2-interface"></a>Rozhraní IVsPackageinstaller2
...
## <a name="ivspackageinstallerevents-interface"></a>Rozhraní IVsPackageInstallerEvents
...
## <a name="ivspackageinstallerprojectevents-interface"></a>Rozhraní IVsPackageInstallerProjectEvents
...
## <a name="ivspackageinstallerservices-interface"></a>Rozhraní IVsPackageInstallerServices
...
## <a name="ivspackagemanagerprovider-interface"></a>Rozhraní IVsPackageManagerProvider
...
## <a name="ivspackagemetadata-interface"></a>Rozhraní IVsPackageMetadata
...
## <a name="ivspackageprojectmetadata-interface"></a>Rozhraní IVsPackageProjectMetadata
...
## <a name="ivspackagerestorer-interface"></a>Rozhraní IVsPackageRestorer
...
## <a name="ivspackagesourceprovider-interface"></a>Rozhraní IVsPackageSourceProvider
...
## <a name="ivspackageuninstaller-interface"></a>Rozhraní IVsPackageUninstaller
...
## <a name="ivspathcontext-interface"></a>Rozhraní IVsPathContext
...
## <a name="ivspathcontext2-interface"></a>Rozhraní IVsPathContext2
...
## <a name="ivspathcontextprovider-interface"></a>Rozhraní IVsPathContextProvider
...
## <a name="ivspathcontextprovider2-interface"></a>Rozhraní IVsPathContextProvider2
...
## <a name="ivsprojectjsontopackagereferencemigrateresult-interface"></a>Rozhraní IVsProjectJsonToPackageReferenceMigrateResult
...
## <a name="ivsprojectjsontopackagereferencemigrator-interface"></a>Rozhraní IVsProjectJsonToPackageReferenceMigrator
...
## <a name="ivssemanticversioncomparer-interface"></a>Rozhraní IVsSemanticVersionComparer
...
## <a name="ivstemplatewizard-interface"></a>Rozhraní IVsTemplateWizard
