...
Který formát správy balíčků je zaměstnán v libovolném daném projektu, závisí na typu projektu a dostupné verzi NuGetu (a/nebo Visual studia). Pokud chcete zkontrolovat, který formát se používá, stačí `packages.config` po instalaci prvního balíčku najít v kořenu projektu. Pokud tento soubor nemáte, hledejte v souboru projektu přímo pro \<PackageReference\> prvek.
...
> [!Tip]
> Mezi různé `nuget.exe` příkazy rozhraní příkazového řádku, například `nuget install` , nepřidáte balíček do seznamu odkazů automaticky. Seznam se aktualizuje při instalaci balíčku pomocí Správce balíčků sady Visual Studio (UI nebo konzole) a pomocí rozhraní příkazového `dotnet.exe` řádku.
...
A konečně hodně úvodní komentáře a příspěvky k této dokumentaci &mdash; stačí vybrat v horní části libovolné stránky příkazy pro **zpětnou vazbu** a **Úpravy** nebo přejít na seznam [úložišť docs](https://github.com/NuGet/docs.microsoft.com-nuget/) a [dokumentace k dokumentaci](https://github.com/NuGet/docs.microsoft.com-nuget/issues) na GitHubu.
