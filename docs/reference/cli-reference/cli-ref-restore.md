...
Stáhne a nainstaluje ze `packages` složky chybějící balíčky. Při použití s NuGet 4.0 + a formátem PackageReference vygeneruje `<project>.nuget.props` `obj` v případě potřeby soubor ve složce. (Soubor může být vynechán ze správy zdrojového kódu.)
...
kde `<projectPath>` určuje umístění řešení `packages.config` nebo souboru. Podrobnosti o chování najdete níže v části [poznámky](#remarks) .
...
| OutputDirectory | Určuje složku, ve které jsou balíčky nainstalované. Pokud není zadána žádná složka, je použita aktuální složka. Vyžaduje se při obnovení `packages.config` souboru se souborem, pokud `PackagesDirectory` se nepoužívá nebo `SolutionDirectory` .|
| PackageSaveMode | Určuje typy souborů, které se uloží po instalaci balíčku: jedna z `nuspec`hodnot `nupkg`, nebo `nuspec;nupkg`. |
| PackagesDirectory | Stejné jako `OutputDirectory`. Vyžaduje se při obnovení `packages.config` souboru se souborem, pokud `OutputDirectory` se nepoužívá nebo `SolutionDirectory` . |
...
3. Při obnovování balíčků pro řešení provede NuGet následující:
    - Načte soubor řešení.
    - Obnoví balíčky na úrovni řešení uvedené `$(SolutionDir)\.nuget\packages.config` v části `packages` do složky.
    - Obnovte balíčky uvedené `$(ProjectDir)\packages.config` v části `packages` do složky. Pro každý zadaný balíček obnovte balíček paralelně, pokud `-DisableParallelProcessing` není zadaný.
