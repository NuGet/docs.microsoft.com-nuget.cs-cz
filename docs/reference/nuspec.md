...
## <a name="project-type-compatibility"></a><span data-ttu-id="9c4f2-116">Kompatibilita typů projektu</span><span class="sxs-lookup"><span data-stu-id="9c4f2-116">Project type compatibility</span></span>
...
<span data-ttu-id="9c4f2-126">V rámci tohoto schématu `.nuspec` má soubor následující obecný tvar:</span><span class="sxs-lookup"><span data-stu-id="9c4f2-126">Within this schema, a `.nuspec` file has the following general form:</span></span>
...
<span data-ttu-id="9c4f2-136">Tyto prvky se musí objevit v rámci `<metadata>` elementu.</span><span class="sxs-lookup"><span data-stu-id="9c4f2-136">These elements must appear within a `<metadata>` element.</span></span>
...
<span data-ttu-id="9c4f2-151">Při nahrávání balíčku do nuget.org `authors` je pole omezeno na 4000 znaků.</span><span class="sxs-lookup"><span data-stu-id="9c4f2-151">When uploading a package to nuget.org, the `authors` field is limited to 4000 characters.</span></span>
...
#### <a name="license"></a><span data-ttu-id="9c4f2-165">license</span><span class="sxs-lookup"><span data-stu-id="9c4f2-165">license</span></span>
...
#### <a name="icon"></a><span data-ttu-id="9c4f2-183">ikona</span><span class="sxs-lookup"><span data-stu-id="9c4f2-183">icon</span></span>
...
#### <a name="requirelicenseacceptance"></a><span data-ttu-id="9c4f2-194">requireLicenseAcceptance</span><span class="sxs-lookup"><span data-stu-id="9c4f2-194">requireLicenseAcceptance</span></span>
...
<span data-ttu-id="9c4f2-208">Při nahrávání balíčku do nuget.org `releaseNotes` je pole omezeno na 35 000 znaků.</span><span class="sxs-lookup"><span data-stu-id="9c4f2-208">When uploading a package to nuget.org, the `releaseNotes` field is limited to 35,000 characters.</span></span>
...
<span data-ttu-id="9c4f2-226">Například:</span><span class="sxs-lookup"><span data-stu-id="9c4f2-226">For example:</span></span>
```xml
<?xml version="1.0"?>
<package xmlns="http://schemas.microsoft.com/packaging/2010/07/nuspec.xsd">
    <metadata>
...
#### <a name="frameworkassemblies"></a><span data-ttu-id="9c4f2-241">frameworkAssemblies</span><span class="sxs-lookup"><span data-stu-id="9c4f2-241">frameworkAssemblies</span></span>
...
### <a name="metadata-attributes"></a><span data-ttu-id="9c4f2-258">atributy metadat</span><span class="sxs-lookup"><span data-stu-id="9c4f2-258">metadata attributes</span></span>
...
<span data-ttu-id="9c4f2-267">Na příkazovém řádku určíte hodnoty tokenu pomocí `nuget pack -properties <name>=<value>;<name>=<value>` .</span><span class="sxs-lookup"><span data-stu-id="9c4f2-267">On the command line, you specify token values with `nuget pack -properties <name>=<value>;<name>=<value>`.</span></span> <span data-ttu-id="9c4f2-268">Například můžete použít token jako `$owners$` a `$desc$` v `.nuspec` a a zadat hodnoty v době balení následujícím způsobem:</span><span class="sxs-lookup"><span data-stu-id="9c4f2-268">For example, you can use a token such as `$owners$` and `$desc$` in the `.nuspec` and provide the values at packing time as follows:</span></span>
...
<span data-ttu-id="9c4f2-355">Následující příklad ukazuje různé varianty `<group>` elementu:</span><span class="sxs-lookup"><span data-stu-id="9c4f2-355">The following example shows different variations of the `<group>` element:</span></span>
...
<span data-ttu-id="9c4f2-409">**Samostatné sestavení specifické pro cílovou architekturu**</span><span class="sxs-lookup"><span data-stu-id="9c4f2-409">**Single assembly specific to a target framework**</span></span>
...
## <a name="including-content-files"></a><span data-ttu-id="9c4f2-413">Zahrnutí souborů obsahu</span><span class="sxs-lookup"><span data-stu-id="9c4f2-413">Including content files</span></span>
...
<span data-ttu-id="9c4f2-421">Soubory obsahu jsou zahrnuty v balíčku pomocí `<files>` elementu a určení `content` složky v `target` atributu.</span><span class="sxs-lookup"><span data-stu-id="9c4f2-421">Content files are included in a package using the `<files>` element, specifying the `content` folder in the `target` attribute.</span></span> <span data-ttu-id="9c4f2-422">Nicméně tyto soubory jsou ignorovány při instalaci balíčku v projektu pomocí PackageReference, který místo toho používá `<contentFiles>` element.</span><span class="sxs-lookup"><span data-stu-id="9c4f2-422">However, such files are ignored when the package is installed in a project using PackageReference, which instead uses the `<contentFiles>` element.</span></span>

<span data-ttu-id="9c4f2-423">Pro maximální kompatibilitu s spotřebou projektů balíček v ideálním případě Určuje soubory obsahu v obou prvcích.</span><span class="sxs-lookup"><span data-stu-id="9c4f2-423">For maximum compatibility with consuming projects, a package ideally specifies the content files in both elements.</span></span>

### <a name="using-the-files-element-for-content-files"></a><span data-ttu-id="9c4f2-424">Použití elementu Files pro soubory obsahu</span><span class="sxs-lookup"><span data-stu-id="9c4f2-424">Using the files element for content files</span></span>

<span data-ttu-id="9c4f2-425">Pro soubory obsahu stačí použít stejný formát jako u souborů sestavení, ale určete `content` jako základní složku v atributu, jak je `target` znázorněno v následujících příkladech.</span><span class="sxs-lookup"><span data-stu-id="9c4f2-425">For content files, simply use the same format as for assembly files, but specify `content` as the base folder in the `target` attribute as shown in the following examples.</span></span>

<span data-ttu-id="9c4f2-426">**Základní soubory obsahu**</span><span class="sxs-lookup"><span data-stu-id="9c4f2-426">**Basic content files**</span></span>
...
<span data-ttu-id="9c4f2-427">**Soubory obsahu s adresářovou strukturou**</span><span class="sxs-lookup"><span data-stu-id="9c4f2-427">**Content files with directory structure**</span></span>
...
<span data-ttu-id="9c4f2-428">**Soubor obsahu specifický pro cílovou architekturu**</span><span class="sxs-lookup"><span data-stu-id="9c4f2-428">**Content file specific to a target framework**</span></span>
...
<span data-ttu-id="9c4f2-429">**Soubor obsahu byl zkopírován do složky s tečkou v názvu**</span><span class="sxs-lookup"><span data-stu-id="9c4f2-429">**Content file copied to a folder with dot in name**</span></span>

<span data-ttu-id="9c4f2-430">V takovém případě NuGet zjistí, že rozšíření v `target` neodpovídá rozšíření v nástroji `src` , a proto zpracovává tuto část názvu `target` jako složku:</span><span class="sxs-lookup"><span data-stu-id="9c4f2-430">In this case, NuGet sees that the extension in `target` does not match the extension in `src` and thus treats that part of the name in `target` as a folder:</span></span>
...
<span data-ttu-id="9c4f2-431">**Soubory obsahu bez rozšíření**</span><span class="sxs-lookup"><span data-stu-id="9c4f2-431">**Content files without extensions**</span></span>

<span data-ttu-id="9c4f2-432">Chcete-li zahrnout soubory bez přípony, použijte `*` `**` zástupné znaky nebo:</span><span class="sxs-lookup"><span data-stu-id="9c4f2-432">To include files without an extension, use the `*` or `**` wildcards:</span></span>
...
<span data-ttu-id="9c4f2-433">**Soubory obsahu s hlubokým umístěním a hlubokou cílovou cestou**</span><span class="sxs-lookup"><span data-stu-id="9c4f2-433">**Content files with deep path and deep target**</span></span>

<span data-ttu-id="9c4f2-434">V tomto případě předpokládá, že vzhledem k příponám souborů ve zdrojové a cílové shodě NuGet předpokládá, že cílem je název souboru, a ne složka:</span><span class="sxs-lookup"><span data-stu-id="9c4f2-434">In this case, because the file extensions of the source and target match, NuGet assumes that the target is a file name and not a folder:</span></span>
...
<span data-ttu-id="9c4f2-435">**Přejmenování souboru obsahu v balíčku**</span><span class="sxs-lookup"><span data-stu-id="9c4f2-435">**Renaming a content file in the package**</span></span>
...
<span data-ttu-id="9c4f2-436">**Vyloučení souborů**</span><span class="sxs-lookup"><span data-stu-id="9c4f2-436">**Excluding files**</span></span>
...
### <a name="using-the-contentfiles-element-for-content-files"></a><span data-ttu-id="9c4f2-437">Použití elementu contentFiles pro soubory obsahu</span><span class="sxs-lookup"><span data-stu-id="9c4f2-437">Using the contentFiles element for content files</span></span>

<span data-ttu-id="9c4f2-438">*NuGet 4.0 + s PackageReference*</span><span class="sxs-lookup"><span data-stu-id="9c4f2-438">*NuGet 4.0+ with PackageReference*</span></span>

<span data-ttu-id="9c4f2-439">Ve výchozím nastavení balíček umístí obsah do `contentFiles` složky (viz níže) a `nuget pack` zahrne všechny soubory v této složce s použitím výchozích atributů.</span><span class="sxs-lookup"><span data-stu-id="9c4f2-439">By default, a package places content in a `contentFiles` folder (see below) and `nuget pack` included all files in that folder using default attributes.</span></span> <span data-ttu-id="9c4f2-440">V takovém případě není nutné zahrnout `contentFiles` uzel do `.nuspec` .</span><span class="sxs-lookup"><span data-stu-id="9c4f2-440">In this case it's not necessary to include a `contentFiles` node in the `.nuspec` at all.</span></span>

<span data-ttu-id="9c4f2-441">Chcete-li určit, které soubory jsou zahrnuty, `<contentFiles>` prvek určuje kolekci `<files>` prvků, které identifikují přesné soubory.</span><span class="sxs-lookup"><span data-stu-id="9c4f2-441">To control which files are included, the `<contentFiles>` element specifies is a collection of `<files>` elements that identify the exact files include.</span></span>

<span data-ttu-id="9c4f2-442">Tyto soubory jsou zadány pomocí sady atributů, které popisují, jak by měly být použity v rámci systému projektu:</span><span class="sxs-lookup"><span data-stu-id="9c4f2-442">These files are specified with a set of attributes that describe how they should be used within the project system:</span></span>

| <span data-ttu-id="9c4f2-443">Atribut</span><span class="sxs-lookup"><span data-stu-id="9c4f2-443">Attribute</span></span> | <span data-ttu-id="9c4f2-444">Popis</span><span class="sxs-lookup"><span data-stu-id="9c4f2-444">Description</span></span> |
| --- | --- |
| <span data-ttu-id="9c4f2-445">**připojit**</span><span class="sxs-lookup"><span data-stu-id="9c4f2-445">**include**</span></span> | <span data-ttu-id="9c4f2-446">Požadovanou Umístění souboru nebo souborů, které mají být zahrnuty, v závislosti na vyloučení určených `exclude` atributem.</span><span class="sxs-lookup"><span data-stu-id="9c4f2-446">(Required) The location of the file or files to include, subject to exclusions specified by the `exclude` attribute.</span></span> <span data-ttu-id="9c4f2-447">Cesta je relativní ke složce, `contentFiles` Pokud není zadána absolutní cesta.</span><span class="sxs-lookup"><span data-stu-id="9c4f2-447">The path is relative to the `contentFiles` folder unless an absolute path is specified.</span></span> <span data-ttu-id="9c4f2-448">Zástupný znak `*` je povolen a dvojitý zástupný znak `**` implikuje hledání rekurzivní složky.</span><span class="sxs-lookup"><span data-stu-id="9c4f2-448">The wildcard character `*` is allowed, and the double wildcard `**` implies a recursive folder search.</span></span> |
| <span data-ttu-id="9c4f2-449">**vyloučení**</span><span class="sxs-lookup"><span data-stu-id="9c4f2-449">**exclude**</span></span> | <span data-ttu-id="9c4f2-450">Seznam souborů nebo vzorů souborů, které mají být vyloučeny z umístění, jsou odděleny středníkem `src` .</span><span class="sxs-lookup"><span data-stu-id="9c4f2-450">A semicolon-delimited list of files or file patterns to exclude from the `src` location.</span></span> <span data-ttu-id="9c4f2-451">Zástupný znak `*` je povolen a dvojitý zástupný znak `**` implikuje hledání rekurzivní složky.</span><span class="sxs-lookup"><span data-stu-id="9c4f2-451">The wildcard character `*` is allowed, and the double wildcard `**` implies a recursive folder search.</span></span> |
| <span data-ttu-id="9c4f2-452">**buildAction**</span><span class="sxs-lookup"><span data-stu-id="9c4f2-452">**buildAction**</span></span> | <span data-ttu-id="9c4f2-453">Akce sestavení, která má být přiřazena položce obsahu pro MSBuild, jako například `Content` ,,, `None` `Embedded Resource` `Compile` atd. Výchozí hodnota je `Compile` .</span><span class="sxs-lookup"><span data-stu-id="9c4f2-453">The build action to assign to the content item for MSBuild, such as `Content`, `None`, `Embedded Resource`, `Compile`, etc. The default is `Compile`.</span></span> |
| <span data-ttu-id="9c4f2-454">**copyToOutput**</span><span class="sxs-lookup"><span data-stu-id="9c4f2-454">**copyToOutput**</span></span> | <span data-ttu-id="9c4f2-455">Logická hodnota označující, zda se mají kopírovat položky obsahu do výstupní složky Build (nebo Publishing).</span><span class="sxs-lookup"><span data-stu-id="9c4f2-455">A Boolean indicating whether to copy content items to the build (or publish) output folder.</span></span> <span data-ttu-id="9c4f2-456">Výchozí hodnotou je hodnota false.</span><span class="sxs-lookup"><span data-stu-id="9c4f2-456">The default is false.</span></span> |
| <span data-ttu-id="9c4f2-457">**Flatten**</span><span class="sxs-lookup"><span data-stu-id="9c4f2-457">**flatten**</span></span> | <span data-ttu-id="9c4f2-458">Logická hodnota označující, zda se mají kopírovat položky obsahu do jediné složky ve výstupu sestavení (true), nebo pro zachování struktury složek v balíčku (false).</span><span class="sxs-lookup"><span data-stu-id="9c4f2-458">A Boolean indicating whether to copy content items to a single folder in the build output (true), or to preserve the folder structure in the package (false).</span></span> <span data-ttu-id="9c4f2-459">Tento příznak funguje pouze v případě, že příznak copyToOutput je nastaven na hodnotu true.</span><span class="sxs-lookup"><span data-stu-id="9c4f2-459">This flag only works when copyToOutput flag is set to true.</span></span> <span data-ttu-id="9c4f2-460">Výchozí hodnotou je hodnota false.</span><span class="sxs-lookup"><span data-stu-id="9c4f2-460">The default is false.</span></span> |

<span data-ttu-id="9c4f2-461">Při instalaci balíčku NuGet aplikuje podřízené prvky `<contentFiles>` od shora dolů.</span><span class="sxs-lookup"><span data-stu-id="9c4f2-461">When installing a package, NuGet applies the child elements of `<contentFiles>` from top to bottom.</span></span> <span data-ttu-id="9c4f2-462">Pokud se stejný soubor shoduje s více položkami, uplatní se všechny položky.</span><span class="sxs-lookup"><span data-stu-id="9c4f2-462">If multiple entries match the same file then all entries are applied.</span></span> <span data-ttu-id="9c4f2-463">Pokud dojde ke konfliktu pro stejný atribut, přepíše položka nejvyšší úrovně nižší položky.</span><span class="sxs-lookup"><span data-stu-id="9c4f2-463">The top-most entry overrides the lower entries if there is a conflict for the same attribute.</span></span>

#### <a name="package-folder-structure"></a><span data-ttu-id="9c4f2-464">Struktura složky balíčku</span><span class="sxs-lookup"><span data-stu-id="9c4f2-464">Package folder structure</span></span>

<span data-ttu-id="9c4f2-465">Projekt balíčku by měl strukturovat obsah pomocí následujícího vzoru:</span><span class="sxs-lookup"><span data-stu-id="9c4f2-465">The package project should structure content using the following pattern:</span></span>

```
/contentFiles/{codeLanguage}/{TxM}/{any?}
```

- <span data-ttu-id="9c4f2-466">`codeLanguages` může `cs` to být, `vb` ,, `fs` `any` nebo malý ekvivalent daného `$(ProjectLanguage)`</span><span class="sxs-lookup"><span data-stu-id="9c4f2-466">`codeLanguages` may be `cs`, `vb`, `fs`, `any`, or the lowercase equivalent of a given `$(ProjectLanguage)`</span></span>
- <span data-ttu-id="9c4f2-467">`TxM` je libovolný moniker platného cílového rozhraní, který podporuje NuGet (viz [cílové architektury](../reference/target-frameworks.md)).</span><span class="sxs-lookup"><span data-stu-id="9c4f2-467">`TxM` is any legal target framework moniker that NuGet supports (see [Target frameworks](../reference/target-frameworks.md)).</span></span>
- <span data-ttu-id="9c4f2-468">Ke konci této syntaxe může být připojena jakákoli struktura složky.</span><span class="sxs-lookup"><span data-stu-id="9c4f2-468">Any folder structure may be appended to the end of this syntax.</span></span>

<span data-ttu-id="9c4f2-469">Například:</span><span class="sxs-lookup"><span data-stu-id="9c4f2-469">For example:</span></span>
...
<span data-ttu-id="9c4f2-470">Prázdné složky se můžou použít `.` k odhlášení o poskytování obsahu pro určité kombinace jazyka a TxM, například:</span><span class="sxs-lookup"><span data-stu-id="9c4f2-470">Empty folders can use `.` to opt out of providing content for certain combinations of language and TxM, for example:</span></span>
...
#### <a name="example-contentfiles-section"></a><span data-ttu-id="9c4f2-471">Příklad oddílu contentFiles</span><span class="sxs-lookup"><span data-stu-id="9c4f2-471">Example contentFiles section</span></span>
...
## <a name="framework-reference-groups"></a><span data-ttu-id="9c4f2-472">Referenční skupiny rozhraní</span><span class="sxs-lookup"><span data-stu-id="9c4f2-472">Framework reference groups</span></span>

<span data-ttu-id="9c4f2-473">*Pouze verze 5.1 + wih PackageReference*</span><span class="sxs-lookup"><span data-stu-id="9c4f2-473">*Version 5.1+ wih PackageReference only*</span></span>

<span data-ttu-id="9c4f2-474">Odkazy na rozhraní jsou koncept .NET Core, který představuje sdílené architektury, jako je WPF nebo model Windows Forms.</span><span class="sxs-lookup"><span data-stu-id="9c4f2-474">Framework References are a .NET Core concept representing shared frameworks such as WPF or Windows Forms.</span></span>
<span data-ttu-id="9c4f2-475">Zadáte-li sdílené rozhraní, balíček zajistí, že všechny jeho závislosti rozhraní jsou zahrnuty do odkazujícího projektu.</span><span class="sxs-lookup"><span data-stu-id="9c4f2-475">By specifying a shared framework, the package ensures that all its framework dependencies are included in the referencing project.</span></span>

<span data-ttu-id="9c4f2-476">Každý `<group>` prvek vyžaduje `targetFramework` atribut a nula nebo více `<frameworkReference>` prvků.</span><span class="sxs-lookup"><span data-stu-id="9c4f2-476">Each `<group>` element requires a `targetFramework` attribute and zero or more `<frameworkReference>` elements.</span></span>

<span data-ttu-id="9c4f2-477">Následující příklad ukazuje nuspec vygenerovaný pro projekt .NET Core WPF.</span><span class="sxs-lookup"><span data-stu-id="9c4f2-477">The following example shows a nuspec generated for a .NET Core WPF project.</span></span>
<span data-ttu-id="9c4f2-478">Mějte na paměti, že ruční vytváření nuspecs obsahujících odkazy na rozhraní se nedoporučuje.</span><span class="sxs-lookup"><span data-stu-id="9c4f2-478">Note that hand authoring nuspecs that contain framework references is not recommended.</span></span> <span data-ttu-id="9c4f2-479">Místo toho zvažte použití sady [targets](msbuild-targets.md) Pack, které budou automaticky odvodit z projektu.</span><span class="sxs-lookup"><span data-stu-id="9c4f2-479">Consider using the [targets](msbuild-targets.md) pack instead, which will automatically infer them from the project.</span></span>
...
## <a name="example-nuspec-files"></a><span data-ttu-id="9c4f2-480">Příklady souborů nuspec</span><span class="sxs-lookup"><span data-stu-id="9c4f2-480">Example nuspec files</span></span>

<span data-ttu-id="9c4f2-481">**Jednoduchá `.nuspec` , která neurčuje závislosti nebo soubory**</span><span class="sxs-lookup"><span data-stu-id="9c4f2-481">**A simple `.nuspec` that does not specify dependencies or files**</span></span>
...
<span data-ttu-id="9c4f2-482">**A `.nuspec` se závislostmi**</span><span class="sxs-lookup"><span data-stu-id="9c4f2-482">**A `.nuspec` with dependencies**</span></span>
...
<span data-ttu-id="9c4f2-483">**A `.nuspec` se soubory**</span><span class="sxs-lookup"><span data-stu-id="9c4f2-483">**A `.nuspec` with files**</span></span>
...
<span data-ttu-id="9c4f2-484">**A `.nuspec` se sestaveními architektury**</span><span class="sxs-lookup"><span data-stu-id="9c4f2-484">**A `.nuspec` with framework assemblies**</span></span>
...
<span data-ttu-id="9c4f2-485">V tomto příkladu jsou nainstalovány následující pro konkrétní cíle projektu:</span><span class="sxs-lookup"><span data-stu-id="9c4f2-485">In this example, the following are installed for specific project targets:</span></span>

- <span data-ttu-id="9c4f2-486">. NET4-> `System.Web` , `System.Net`</span><span class="sxs-lookup"><span data-stu-id="9c4f2-486">.NET4 -> `System.Web`, `System.Net`</span></span>
- <span data-ttu-id="9c4f2-487">. Profil klienta NET4 – > `System.Net`</span><span class="sxs-lookup"><span data-stu-id="9c4f2-487">.NET4 Client Profile -> `System.Net`</span></span>
- <span data-ttu-id="9c4f2-488">Silverlight 3 – > `System.Json`</span><span class="sxs-lookup"><span data-stu-id="9c4f2-488">Silverlight 3 -> `System.Json`</span></span>
- <span data-ttu-id="9c4f2-489">WindowsPhone – > `Microsoft.Devices.Sensors`</span><span class="sxs-lookup"><span data-stu-id="9c4f2-489">WindowsPhone -> `Microsoft.Devices.Sensors`</span></span>
