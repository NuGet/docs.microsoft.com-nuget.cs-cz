...
Vytvoření balíčku NuGet z knihovny tříd .NET Framework zahrnuje vytvoření knihovny DLL v aplikaci Visual Studio ve Windows a následné použití nuget.exe nástroje příkazového řádku k vytvoření a publikování balíčku.
...
1. V aplikaci Visual Studio zvolte **soubor > nový > projekt**, vyberte uzel **Visual C#** , vyberte šablonu knihovny tříd (.NET Framework), pojmenujte projekt AppLogger a klikněte na tlačítko **OK**.
...
Když je knihovna DLL v rukou a nastavené vlastnosti projektu, můžete nyní použít `nuget spec` příkaz k vygenerování počátečního `.nuspec` souboru z projektu. Tento krok zahrnuje relevantní náhradní tokeny k vykreslování informací ze souboru projektu.

`nuget spec`Pro vygenerování počátečního manifestu se spustí jenom jednou. Při aktualizaci balíčku můžete buď změnit hodnoty v projektu, nebo přímo upravit manifest.

1. Otevřete příkazový řádek a přejděte do složky projektu obsahující `AppLogger.csproj` soubor.

1. Spusťte následující příkaz: `nuget spec AppLogger.csproj` . Když zadáte projekt, NuGet vytvoří v tomto případě manifest, který se shoduje s názvem projektu `AppLogger.nuspec` . Obsahuje také náhradní tokeny v manifestu.
...
1. Z příkazového řádku ve složce, která obsahuje váš `.nuspec` soubor, spusťte příkaz `nuget pack` .
